...
- 新增 `playbooks/multimedia-prompt-pack.md`，并接入 `PLAYBOOKS.md` 与 `self-evolution-log/roadmap.md`，把多媒体复杂任务从“brief 已有但 prompt 仍是一次性大段文本”升级为 `shared guardrails -> variant prompts -> acceptance checklist -> next revision axis` 的可复用首轮生成包，减少生图 / 视频 / 图生视频任务里的 prompt 返工与版本比较失焦
...
- ??????? playbooks/browser-research-briefing.md?????????????о??????????ü???????????? PLAYBOOKS.md ?? TOOLS.md????????????С??λ?о??????????????????????????????
...
- ?? completion proof ????? `playbooks/windows-file-dialog-handoff.md`???? Open / Save / Save As / Select Folder ??????????????????? + ???y??? + handoff-ready????????????????? / ???? / ?????·?е????????
...
- 新增 `self-evolution-log/ideas/career-form-fill-pack-handoff-template.md`，并将 `Career Prep Handoff -> Career Form Fill Pack -> web-form-execution` 接入 `playbooks/career-prep-handoff.md` 与 `roadmap.md`，降低官网申请类复杂表单中边翻资料边填、上传目标不唯一、敏感字段混填和 draft-ready 边界不清造成的返工
