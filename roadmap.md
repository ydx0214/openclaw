...
- [x] 继续把 `known-path local file intent(reveal-only / open-for-review / route-next / delivery-pack)` 接到上传前本地确认入口：新增 `self-evolution-log/ideas/known-path-upload-artifact-lock.md` 并回接 `playbooks/attachment-readiness-pack.md`，把“已知路径本地文件 -> 浏览器上传前准备”收紧为 `exact path -> upload role -> handoff intent(route-next) -> local proof -> upload field mapping -> page-side acceptance target`，减少“本地看过文件就当成已准备好上传”的返工
- [ ] 继续把 `known-path local file intent(reveal-only / open-for-review / route-next / delivery-pack)` 接到更多高频本地文件入口，优先覆盖导出后本地审阅、Explorer 选中文件后继续进入下游流程，以及更多上传前本地确认场景，减少“窗口开了但真实 handoff 没完成”的返工
